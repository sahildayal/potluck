...
**Live:** [app](https://potluck-2po.pages.dev) · [API](https://potluck-api-pcct.onrender.com/health)

> The PWA is on Cloudflare Pages, which is always warm. The API is on Render's free tier, which sleeps after 15 minutes idle — so the app appears instantly but the first request after a quiet period waits 30–60 seconds for the container. A [scheduled ping](.github/workflows/keep-warm.yml) keeps it awake during waking hours; going 24/7 would consume the entire 750-hour monthly allowance. See [The $0 constraint](#-the-0-constraint).
...
### Cloudflare Pages + Render (live)

The PWA is a static bundle on Cloudflare Pages (`pnpm deploy:pages`), and the API is a Render web service. They are on different origins, which is why the session cookie is `SameSite=None; Secure` and why `APP_URL` accepts a comma-separated allowlist — moving the frontend between hosts would otherwise break the old origin the moment the new one worked.
...
One honest caveat if you `EXPLAIN` it yourself: at the catalog's current size the planner picks a sequential scan over the GIN index, and it is right to — the whole table is smaller than the index lookup is worth. Forcing `enable_seqscan = off` confirms the index is present and used (`Bitmap Index Scan on catalog_recipes_search_idx`). The index earns its place as the table grows, not today.
